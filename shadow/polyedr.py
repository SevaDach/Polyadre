...
def pl(tochki1, c):
    ...
    for i in range(1, len(tochki) - 1):
        ...
        # bb = R3(tochki[i-1].x-tochki[i].x,
        # tochki[i-1].y-tochki[i].y, tochki[i-1].z-tochki[i].z)
        cc = 0.5 * sqrt(v2v1.cross(v3v1).x * v2v1.cross(v3v1).x
                        + v2v1.cross(v3v1).y * v2v1.cross(v3v1).y
                        + v2v1.cross(v3v1).z * v2v1.cross(v3v1).z)
        ppl += cc
    return ppl
...
class Polyedr:
            ...
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    for i in range(len(vertexes)):
                        if vertexes[i].x in self.plohiexxx and \
                           vertexes[i].y in self.plohieyyy and \
                           vertexes[i].z in self.plohiezzz:
                            # print('suka')
    ...
    def draw(self, tk):
        ...
        print('Сумма площадей граней, все вершины которых не являются \
        "хорошими" точками =', round(self.ploschad, 1))
